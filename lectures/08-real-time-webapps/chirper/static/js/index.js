...
(function () {
  ...
  function update() {
    ...
    return ChirpApiService.getChirps().then((response) => {
      ...
      chirps.forEach(function (chirp) {
        const newChirpComponent = createChirpComponent(chirp);
        // prepend to DOM
        document.querySelector(".chirps").append(newChirpComponent);
      });
    });
  }
  ...
  window.addEventListener("DOMContentLoaded", function () {
    ...
    // connect to socket
    const socket = io();
    // whenever we get the "chirp" event with the JSON payload...
    socket.on("chirp", function (chirp) {
      const newChirpComponent = createChirpComponent(chirp);
      // prepend to DOM
      document.querySelector(".chirps").prepend(newChirpComponent);
    });
    ...
    document
      ...
      .addEventListener("submit", function (e) {
        ...
        ChirpApiService.addChirp(formData)
          // .then(update)  this is not longer used because we are updating messages using socket.
          .then(() => {
            // clear input field
            e.target.reset();
            // enable submit button
